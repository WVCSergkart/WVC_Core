...
namespace WVC_UltraExpansion
{
	...
	namespace ImplantGenerator
	{
		...
		public static class TemplatesUtility
		{
			...
			public static void GetOrCreateStorageModFolders(ImplantGeneratorDef implantGeneratorDef, out string path)
			{
				DirectoryInfo directoryInfo = new(Path.Combine(new DirectoryInfo(GenFilePaths.ModsFolderPath).ToString(), "AutoGenerated"));
				if (!directoryInfo.Exists)
				{
					directoryInfo.Create();
				}
				DirectoryInfo directoryInfo4 = new(Path.Combine(directoryInfo.ToString(), "Implants"));
				if (!directoryInfo4.Exists)
				{
					directoryInfo4.Create();
				}
				path = Path.Combine(directoryInfo4.ToString(), implantGeneratorDef.defName);
				DirectoryInfo directoryInfo5 = new(path);
			...
			public static XElement GenerateHediffDefFile(HediffDef def, ImplantGeneratorDef generatorDef)
			{
				...
				foreach (var stage in def.stages)
				{
					...
					if (!stage.statOffsets.NullOrEmpty())
					{
						...
						{
							statOffsets.Add(new XElement(statOffset.stat.defName, statOffset.value));
						}
					}
