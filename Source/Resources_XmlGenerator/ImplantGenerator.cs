	...
	public static class PostInitializationDefGenerator
	{
		...
		static PostInitializationDefGenerator()
		{
			foreach (ImplantGeneratorDef generatorDef in DefDatabase<ImplantGeneratorDef>.AllDefsListForReading)
			{
				...
				TemplatesUtility.GetOrCreateStorageModFolders(generatorDef, out string path);
				TemplatesUtility.GetOrCreateStorageModFolders(generatorDef, out string path1);
				TemplatesUtility.GetOrCreateStorageModFolders(generatorDef, out string path2);
