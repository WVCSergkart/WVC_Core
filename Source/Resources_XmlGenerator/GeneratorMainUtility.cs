...
namespace WVC_UltraExpansion
{
	...
	namespace ImplantGenerator
	{
		...
		public static class GeneratorMainUtility
		{
			...
			{
				XElement xElement = new("RecipeDef", new XAttribute("ParentName", "WVC_Ultra_ImplantSurgeryDef_Base"), new XElement("defName", def.defName), new XElement("label", def.label), new XElement("description", def.description), new XElement("jobString", def.jobString), new XElement("descriptionHyperlinks"), new XElement("appliedOnFixedBodyParts"), new XElement("addsHediff", def.addsHediff.defName), new XElement("ingredients"), new XElement("fixedIngredientFilter"));
				XElement descriptionHyperlinks = xElement.Element("descriptionHyperlinks");
				foreach (var i in def.descriptionHyperlinks)
				{
					if (i.def is ThingDef)
